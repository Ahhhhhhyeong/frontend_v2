    ...
    return (
        <div className={styles.div}>
            ...
            <div className={styles.header}>
                <button onClick={() => navigate('/')} className={styles.headerButton}><ChevronLeftIcon /></button>
            ...
            <div className={styles.productList}>
                {products.length > 0 ? (
                    products.map((product) => (
                        ...
                        <div key={product.id} className={styles.card} onClick={() => navigate(`/product-detail/${product.id}`)}>
                            <img className={styles.imgIcon1} src={product.mainImage || 'https://placehold.co/92x92'} alt={product.productName} />
                            <div className={styles.info}>
                                <b className={styles.kg}>{product.productName}</b>
                                <div className={styles.price}>
                                    {product.discount && <div className={styles.div10}>{product.discount}%</div>}
                                    <div className={styles.div7}>{product.options?.[0]?.price ? `${new Intl.NumberFormat('ko-KR').format(product.options[0].price * (1 - (product.discount || 0) / 100))}원` : ''}</div>
                                </div>
                            </div>
                        </div>
                    ))
                ) : (
                    <div className={styles.emptyMessage}>등록된 상품이 없습니다. <br /> '+' 버튼을 눌러 상품을 추가해보세요.</div>
                )}
            </div>
